    ...
    <div style={{ width:'100vw', height:'100vh', display:'flex', flexDirection:'column', background:'var(--bg-primary)', overflow:'hidden' }}>
      ...
      <div className="nav-bar" style={{ margin:'12px 24px 0', flexShrink:0 }}>
        <div className="nav-logo" style={{ fontSize: 18 }}>
          ...
          GitHub Actions — Automation & Post-Deployment
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <button className="nav-btn" onClick={() => go(cur - 1)} disabled={cur === 0} style={{ fontSize:15, padding:'8px 16px' }}>← Prev</button>
          <span className="slide-counter" style={{ fontSize:14 }}>{cur + 1} / {total}</span>
          <button className="nav-btn" onClick={() => go(cur + 1)} disabled={cur === total - 1} style={{ fontSize:15, padding:'8px 16px' }}>Next →</button>
        </div>
      </div>
      ...
      {/* Slide label */}
      <div style={{ textAlign:'center', fontSize:14, color:'var(--text-muted)', marginTop:6, flexShrink:0 }}>
        {SLIDES[cur].label}
      </div>
